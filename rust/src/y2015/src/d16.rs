...
impl AocSolution for Day16 {
    ...
    fn part1(input: &Self::Input) -> impl ToSolution {
        input
            .iter()
            .find(|s| {
                s.children.is_none_or(|c| c == CRITERIA.children)
                    && s.cats.is_none_or(|c| c == CRITERIA.cats)
                    && s.samoyeds.is_none_or(|c| c == CRITERIA.samoyeds)
                    && s.pomeranians.is_none_or(|c| c == CRITERIA.pomeranians)
                    && s.akitas.is_none_or(|c| c == CRITERIA.akitas)
                    && s.vizslas.is_none_or(|c| c == CRITERIA.vizslas)
                    && s.goldfish.is_none_or(|c| c == CRITERIA.goldfish)
                    && s.trees.is_none_or(|c| c == CRITERIA.trees)
                    && s.cars.is_none_or(|c| c == CRITERIA.cars)
                    && s.perfumes.is_none_or(|c| c == CRITERIA.perfumes)
            })
            .unwrap()
            .number
    }
